...
function renderSession(s, labels) {
  ...
  const fullTurns = s.entries.map((e, i) => {
    ...
    let displayBot = e.bot;
    const nextMsg = s.entries[i + 1];
    if (nextMsg) {
      const nextRw = parseRewrite(nextMsg.user);
      if (nextRw && nextRw.removed) {
         const r = nextRw.removed.trim();
         if (r) {
           const idx = displayBot.lastIndexOf(r);
           if (idx !== -1) {
              displayBot = displayBot.substring(0, idx);
           }
         }
      }
    }
    ...
    return `
      ...
      <div class="chat-row bot">
        <div class="bubble clod-markdown">${esc(displayBot)}</div>
